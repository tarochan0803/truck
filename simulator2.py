...
class GeoJsonTruckCanvas(tk.Canvas):
    ...
    def calculate_lateral_force(self):
        ...
        lateral_slip = math.tan(self.truck_steering) * self.wheelBase_m
    ...
    def check_vehicle_fit(self, during_sim=False):
        ...
        トラックの四隅が対象道路（選択されたGeoJSON道路）のバッファポリゴンに
        ...
        if dist_m < (total_length_m * 0.5):
         ...
         # 対象道路のLineString取得（選択された道路または最初の道路）
        feats = self.geojson_data.get("features", [])
        if not feats:
            return False

         # 選択された道路がある場合はそれを使用、ない場合は最初の道路を使用
        if self.selected_road_featID is not None and self.selected_road_featID < len(feats):
            feat = feats[self.selected_road_featID]
        else:
            feat = feats[0]
...
class DynamicPathPlanner:
    ...
    def find_path(self, start, goal):
        ...
        for _ in range(1000):
            if self.obstacles:
                rand = random.choice([goal] + [tuple(random.uniform(min(coord), max(coord)) for coord in zip(*self.obstacles))])
            else:
                rand = goal
...
class MainApp:
    def __init__(self, root):
        ...
        addparam("幅(m):", "2.5", "w")
        # 以下のパラメータはUI上に出さない(隠す)ならコメントアウト可
        ...
        # ① まずはキャンバスを先に生成する
        self.canvas = GeoJsonTruckCanvas(main_fr)  # <-- GeoJsonTruckCanvasクラスが定義済みであること
        ...
        # ここで初めて canvas にアクセスしてOK
        self.canvas.maxSteeringAngle_deg = self.hidden_params['steer']
        self.canvas.vehicleSpeed_m_s     = self.hidden_params['spd']
        self.canvas.lookahead_m          = self.hidden_params['look']
        self.canvas.max_steer_rate       = deg_to_rad(self.hidden_params['steer_rate'])
        self.canvas.vehicle_mass         = self.hidden_params['mass']
        self.canvas.max_accel            = self.hidden_params['max_accel']
        self.canvas.max_brake            = self.hidden_params['max_brake']

        # ----------------------------
        # PID/SMCなどの制御パラメータを、キャンバスの変数に代入(仮)
        # (必要に応じてUI化してください)
        # ----------------------------
        self.canvas.pid_kp      = 0.8
        self.canvas.pid_ki      = 0.01
        self.canvas.pid_kd      = 0.05
        self.canvas.smc_lambda  = 0.5
        self.canvas.smc_eta     = 0.2
        self.canvas.speed_kp    = 1.0
        self.canvas.speed_ki    = 0.1
        self.canvas.speed_kd    = 0.05

        # シナリオ管理
        self.scenario_manager = ScenarioManager()

        # パス更新時に道路リストを更新するイベント
    ...
    def on_reset(self):
        """シミュレーションリセット"""
        self.apply_params()      # Entryの値をcanvasに反映
        self.canvas.path.clear() # パスもクリア
    ...
    def apply_params(self):
        ...
        # 下記は隠しパラメータ扱いなら固定でもよい
        st          = self.hidden_params['steer']
        spd         = self.hidden_params['spd']
        lk          = self.hidden_params['look']
        steer_rate  = self.hidden_params['steer_rate']
        mass_       = self.hidden_params['mass']
        max_accel_  = self.hidden_params['max_accel']
        max_brake_  = self.hidden_params['max_brake']
    ...
    def populate_road_list(self, event=None):
        """Canvas が <<PathUpdated>> を発行した時に呼ばれ、リスト表示更新"""
    ...
    def on_road_apply(self):
        """編集結果をcanvas.geojson_dataに反映して再描画"""
        if not self.canvas.geojson_data:
            return
        feats = self.canvas.geojson_data["features"]
